...
namespace ZHFSM
{
    ...
    public class StateMachineInspector : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            reorderableList =
                new ReorderableList(helper.stateMachineData.services, typeof(string), true, false, true, true);
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.Space();
            EditorGUILayout.BeginHorizontal();
            EditorGUI.BeginDisabledGroup(!helper.HFSMController.controllerConfig.DisperseGenerate ||
                                         helper.HFSMController.controllerConfig.DisperseAll);
            EditorGUILayout.LabelField(new GUIContent("independent", "Generate script files independently"),
                GUILayout.Width(80));
            helper.stateMachineData.independentGenerate = GUILayout.Toggle(helper.stateMachineData.independentGenerate,
                "", GUILayout.Width(20));
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.EndHorizontal();
        ...
        protected override void OnHeaderGUI()
        {
            ...
            GUILayout.Label(
                EditorGUIUtility.IconContent("icons/processed/unityeditor/animations/animatorstate icon.asset"),
        ...
        private void DrawElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (isRenaming && reorderableList.index == index)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    helper.HFSMController.RenameService(service, tempname, helper.stateMachineData.id);
                    isRenaming = false;
                }
            }
            else
            ...
            else
            {
                ...
                string text = service.serviceType == ServiceType.CustomInterval ? "(毫秒)" : string.Empty;
                EditorGUI.LabelField(rects[0], service.serviceType + text);
                service.customInterval = EditorGUI.LongField(rects[1], service.customInterval);
            }
        }
