...
namespace ZHFSM
{
    public class GraphStateLayer : GraphLayer
    {
        ...
        public override void ProcessEvent()
        {
            ...
            if (Event.current.keyCode == KeyCode.Delete && this.context.selectedStates != null &&
                this.context.selectedStates.Count > 0)
            {
                DeleteNode();
                this.editorWindow.Repaint();
            }
        ...
        private void CreateMenu(StateBaseData item)
        {
            ...
            else
            {
                ...
                genericMenu.AddItem(new GUIContent("Set DefaltState"), false, () => { SetDefaultState(item); });
            ...
            if (GetTransfromRect(data.position).MouseOn())
            {
                if (EventExtension.IsMouseDown(0) || EventExtension.IsMouseDown(1))
                {
                    ...
                    if (data.stateType == StateType.State)
                        ...
                        StateMachineInspectorHelper.instance.Inspector(context.HFSMController,
                            data as StateMachineData);
                    //是否在预览添加过渡
                    if (this.context.isPreviewTransition)
                    {
                        //添加过渡
                        this.context.HFSMController.CreateTransition(context.currentStateMachine, context.preFrom,
                            data);
        ...
        private void DrawState(StateBaseData data)
        {
            if (data != null)
            {
                ...
                {
                    bool isSM = data.stateType == StateType.StateMachine;

                    // 计算整体宽度
                    float barWidth = rect.width;
                    float startX = rect.x;
                    float startY = rect.y + rect.height * 3 / 4;
                    if (isSM)
                    {
                        startX += barWidth * 0.12f;
                        barWidth *= 0.78f;
                        startY += rect.height * 1 / 4;
                    }

                    runStateProcessRect.Set(startX, startY, barWidth, rect.height / 4);
